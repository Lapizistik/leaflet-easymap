...
/*
 * SVG-based markers for geojson including support for maki-markers.
 * These are all templates for L.Util.template().
 */
const svgTemplate =
`<svg style="width:{iconwidth};" 
      viewBox="-10.5 -30.5 21 31.4" 
      xmlns="http://www.w3.org/2000/svg">
  <path d="m 0,0 c 0,0 -10,-15 -10,-20 0,-5 5,-10 10,-10 5,0 10,5 10,10 C 10,-15 0,0 0,0 Z"
        style="fill:{marker-color};stroke:{stroke};stroke-width:{stroke-width};" />
  {inner}
</svg>`;
...
const svgShadowTemplate =
`<svg style="width:{iconwidth};" 
      xmlns="http://www.w3.org/2000/svg" 
      viewBox="0 0 28 28">
  <defs>
    <filter id="f" height="1.7" y="-0.2" width="1.4" x="-0.2">
      <feGaussianBlur stdDeviation="1.5491099" />
    </filter>
  </defs>
  <path d="m 2,18 c 1.4,-2 1,-3.5 3,-6.5 1.5,-3 6.8,-7 10,-8 3,-1 5,-0.8 7,0 1.6,0.9 3.5,1.8 3,3 -0.4,1 0,3 -2,5 -2,1.8 -5.7,3 -9,4 -3.4,1 -8.8,2 -11,3 z"
        style="opacity:0.1;fill:#000;stroke:none;filter:url(#f);" />
</svg>`;
...
function addMakiSVG(div, symbol, params) {
		...
		/*
		 * We don't test for failed requests as we cannot do anything about it anyway
		 * (besides logging perhaps).
		 */
...
function inner(div, params, symbol) {
...
/*
 * The SVGIcon is a special type of DivIcon where the html is a SVG element
 */
const SVGIcon = L.DivIcon.extend({
		...
		initialize: function(options) {
				...
				// we assume a SVGIcon to be centered horicontally and
				...
				if (!isNaN(options.shadowSize)) {
						// if the shadowsize is given as single number we compute its
